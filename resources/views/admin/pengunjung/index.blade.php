@extends('admin.dashboard')


@section('content')
    <div class="card-modern">
        <div class="card-header-custom d-flex justify-content-between align-items-center flex-wrap">
            <h5><i class="fas fa-user-check text-primary me-2"></i>Data Pengunjung</h5>
            <a class="btn btn-admin btn-admin-success" href="{{ route('pengunjung.export_excel',['bulan' => $bulan, 'tahun' => $tahun]) }}" target="_blank">
                <i class="fas fa-file-excel"></i> Export Excel
            </a>
        </div>
        <div class="card-body-custom">
            <!-- Year tabs -->
            <ul class="nav nav-tabs mb-3" role="tablist">
                @foreach ($tahun_visitor as $item_tahun)
                    <li class="nav-item">
                        <button type="button" class="nav-link {{ $item_tahun->year == $tahun ? 'active' : '' }}" role="tab" data-bs-toggle="tab"
                            data-bs-target="#tahun-{{ $item_tahun->year }}" aria-selected="true">
                            {{ $item_tahun->year }}
                        </button>
                    </li>
                @endforeach
            </ul>

            <div class="tab-content">
            @foreach ($tahun_visitor as $item_tahun)
                <div class="tab-pane fade show {{ $item_tahun->year == $tahun ? 'active' : '' }}" id="tahun-{{ $item_tahun->year }}" role="tabpanel">
                    <div class="d-flex justify-content-between align-items-center flex-wrap mb-3">
                        <h5 class="fw-bold mb-0">Bulan {{ bulan_indo($bulan) }} Tahun {{ $item_tahun->year }}</h5>
                        <form action="{{ route('pengunjung.index') }}" method="GET">
                            <input type="hidden" name="tahun" value="{{ $item_tahun->year }}">
                            <select name="bulan" class="form-control-modern" onchange="this.form.submit()">
                                @for ($i = 1; $i <= 12; $i++)
                                    <option value="{{ $i }}" {{ $bulan == $i ? 'selected' : '' }}>
                                        {{ bulan_indo($i) }}
                                    </option>
                                @endfor
                            </select>
                        </form>
                    </div>
                    <div class="table-responsive">
                        <table class="table table-admin table-datatable">
                            <thead>
                                <tr>
                                    <th>No</th>
                                    <th>Nama</th>
                                    <th>Alamat</th>
                                    <th>Layanan</th>
                                    <th>Tanggal</th>
                                </tr>
                            </thead>
                            <tbody>
                                @foreach ($item_tahun->pengunjung as $no => $item_pengunjung)
                                    <tr>
                                        <td>{{ $no+1 }}</td>
                                        <td class="fw-semibold">{{ $item_pengunjung->nama }}</td>
                                        <td>{{ $item_pengunjung->alamat }}</td>
                                        <td>{{ $item_pengunjung->layanan->nama_layanan }}</td>
                                        <td>{{ date('d M Y',strtotime($item_pengunjung->created_at)) }}</td>
                                    </tr>
                                @endforeach
                            </tbody>
                        </table>
                    </div>
                </div>
            @endforeach
            </div>
        </div>
    </div>
@endsection
